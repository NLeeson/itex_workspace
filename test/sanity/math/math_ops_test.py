...
class DivAndModTest(test_util.TensorFlowTestCase):
  ...
  def testRealDiv_Complex64(self):
    nums = np.arange(-10, 10, .25).astype(np.complex64).reshape(80, 1)
    divs = np.arange(-3, 0, .25).astype(np.complex64).reshape(1, 12)
    tf_result = math_ops.realdiv(nums, divs)
    np_result = np.divide(nums, divs)
    self.assertAllClose(tf_result, np_result)

  def testRealDiv_Complex128(self):
    nums = np.arange(-10, 10, .25).astype(np.complex128).reshape(80, 1)
    divs = np.arange(-3, 0, .25).astype(np.complex128).reshape(1, 12)
    tf_result = math_ops.realdiv(nums, divs)
    np_result = np.divide(nums, divs)
    self.assertAllClose(tf_result, np_result)  
  ...
  # TODO(itex): Enable this corner UT, once we find solution to get correct
  # answer for this overflow UT
...
class XdivyTest(test_util.TensorFlowTestCase):
  ...
  def testXdivyNoZero(self):
    for dtype in [dtypes.float16, dtypes.float32, dtypes.float64, dtypes.complex64, dtypes.complex128]:
      x = constant_op.constant([[0.1, 0.2, 3.5], [-2., -5., 30.]], dtype=dtype)
      y = constant_op.constant([[0.1, 0.2, 3.5], [3.1, 4., 2.]], dtype=dtype)
      with test_util.use_gpu():
