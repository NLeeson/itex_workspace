...
def gpu_device_name():
  """Returns the name of a XPU device if available or a empty string.
  ...
  ...       self.skipTest("test is only applicable on XPU")
  ...
...
def run_in_graph_and_eager_modes(func=None,
  ...
  Args:
    ...
    use_gpu: If True, attempt to run as many operations as possible on XPU.
...
def run_gpu_only(func=None):
  """Execute the decorated test only if a XPU is available.

  This function is intended to be applied to tests that require the presence
  of a XPU. If a XPU is absent, it will simply be skipped.
...
def run_cuda_only(func=None):
  ...
  This function is intended to be applied to tests that require the presence
  of a CUDA XPU. If a CUDA XPU is absent, it will simply be skipped.
  ...
  def decorator(f):
    ...
    def decorated(self, *args, **kwargs):
      if not is_gpu_available(cuda_only=True):
        self.skipTest("Test requires CUDA XPU")
...
def run_gpu_or_tpu(func=None):
  """Execute the decorated test only if a physical XPU or TPU is available.

  This function is intended to be applied to tests that require the presence
  of a physical XPU or TPU. It complies with the following rules:
  - If a XPU is available, the test will run on the XPU.
  - If a XPU is absent and a TPU is available, the test will run on the TPU.
  - If both XPU and TPU are absent, the test will be skipped.
  ...
  def decorator(f):
    ...
    def decorated(self, *args, **kwargs):
      if config.list_physical_devices("XPU"):
        return f(self, "XPU", *args, **kwargs)

      if config.list_physical_devices("TPU"):
        return f(self, "TPU", *args, **kwargs)

      self.skipTest("Test requires XPU or TPU")
...
def run_in_native_and_block_format(func=None):
  """Execute the decorated test with native and block format.
  """

  def decorator(f):
    if tf_inspect.isclass(f):
      raise ValueError(
          "`run_in_native_and_block` only supports test methods. "
          "Did you mean to use `run_all_in_native_and_block_format`?")
...
def run_all_in_native_and_block_format(cls):
  """Execute all test methods in the given class with and without eager."""
  base_decorator = run_in_native_and_block_format
  for name in dir(cls):
    if (not name.startswith(unittest.TestLoader.testMethodPrefix) or
        name == "test_session"):
      continue
...
# is_gpu_available will return true only with XPU device + GPU backend
@deprecation.deprecated(None,
                        "Use `tf.config.list_physical_devices('XPU')` instead.")
@tf_export("test.is_gpu_available")
def is_gpu_available(cuda_only=False, min_cuda_compute_capability=None):
  """Returns whether TensorFlow can access a XPU.

  Warning: if a non-XPU version of the package is installed, the function would
  ...
  return true when a XPU device is available irrespective of whether TF was
  ...
  Returns:
    True if a XPU device of the requested kind is available.
  """
...
class TensorFlowTestCase(googletest.TestCase):
  ...
  def session(self, graph=None, config=None, use_gpu=True, force_gpu=False):
    ...
    `force_gpu` is True, all ops are pinned to `/device:XPU:0`. Otherwise, if
    `use_gpu` is True, TensorFlow tries to run as many ops on the XPU as
    ...
    Args:
      ...
      config: An optional config_pb2.ConfigProto to use to configure the
        session.
      use_gpu: If True, attempt to run as many ops as possible on XPU.
      force_gpu: If True, pin all ops to `/device:XPU:0`.
