...
| ITEX_OMP_THREADPOOL    | `1` | Default Python CPU builds ship both oneDNN libraries. `1` loads OpenMP (`libonednn_cpu_so.so`) and conservatively sets `TF_NUM_INTEROP_THREADS`. `0` loads Eigen THREADPOOL (`libonednn_cpu_eigen_so.so`) and attaches oneDNN to TensorFlow's Eigen pool. Builds configured with `--define=build_with_threadpool=true` always use THREADPOOL and ignore `1`.| 
